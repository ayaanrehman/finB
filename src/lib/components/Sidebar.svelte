<script>
	...
	function filterDocuments() {
		docs = documents.filter((document) =>
	...
	<ul class="ulstat">
		...
		{#if li}
			<li class="dl">
				...
				>
					<span style="font-size:normal;color:gold!important;"
				...
				{#if docs}
					{#if showDocuments}
						<ul>
							<input
								style="background-color: rgba(83, 82, 82, 0.9);; color: wheat;"
								type="text"
								bind:value={searchInput}
								placeholder="Search the Data Lake..."
							/>
							{#each docs as document, i}
								<li>
									<a href="/documents/{document.slug}" class="titledoc" class:active={pageUrl == "/documents/" + document.slug}
										>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{document.title}.pdf</a
									>
								</li>
							{/each}
						</ul>
					...
					{#if showDocuments}
						<ul>
							...
							{#each documents as document, i}
								<li>
									<a href="/documents/{document.slug}" class="titledoc" class:active={pageUrl == "/documents/one"}
										>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{document.title}.csv</a>
								</li>
							{/each}
						</ul>
					{/if}
...
<style lang="scss">
	...
	.titledoc:hover, .titledoc.active {
